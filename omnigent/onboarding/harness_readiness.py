...
def _harness_availability(canonical: str) -> HarnessAvailability:
    ...
    if install_key is not None:
        state = _cli_family_availability(canonical, install_key)
        # Cursor native historically reported a bare ``False`` for a missing
        # binary, so legacy web clients render their "install & login" label.
        # Keep that shape while surfacing an outdated binary or missing login as
        # the new structured reasons.
        if canonical in _CURSOR_NATIVE_HARNESSES and state == HARNESS_BINARY_MISSING:
            return False
        return state
